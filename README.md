...
- [ ] **GitOps with Flux CD**:
  - [x] Bootstrap FluxCD on `dev` and `prod` environments.
...
- Bare-metal installation is tricky since I had to boot the machines via USB at the same time. It would be better next time to evaluate if it's worth it to boot the machines via PXE to roll out the same Talos image to all machines at the same time.
- During the Talos apply configuration, I had to provide where to install the OS; on the bare metal, this is not standardized. For instance, on my control plane, the first device is /dev/nvme0n1, but on the worker, it was /dev/sdb.
